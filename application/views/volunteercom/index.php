    ...
    <?php if (get_admin_login()->is_level != '4') { ?>
        <div class="panel-heading">
            <span class="panel-title">
                ...
                <button type="button" class="btn btn-sm btn-primary" onclick="location.href = '<?php echo site_url('volunteercom/add'); ?>';">
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?php echo $icon ?></td>
                        <td>
                            <button type="button" class="btn btn-xs btn-labeled btn-info" onclick="location.href = '<?php echo site_url('volunteercom/edit/' . $row->id) ?>';">
...
<script type="text/javascript">
    function set_status(status) {
        ...
        if (chkId != '') {
            $('#frm #id').val(chkId);
            var action = '<?php echo site_url('volunteercom/set_status') ?>/' + status;
            // compute action here...
    ...
    function data_delete() {
        ...
        if (chkId != '') {
            if (confirm('What do you want to delete?')) {
                $('#frm #id').val(chkId);
                var action = '<?php echo site_url('volunteercom/data_delete') ?>';
                // compute action here...
                $('#frm').attr('action', action);
                $('#frm').submit();
            }
