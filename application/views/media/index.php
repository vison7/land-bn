    ...
    <form id="frm-search" name="frm-search" method="post">
        <div class="col-lg-3">
            <select class="form-control" name="cate_id" id="cate_id">
                ...
                <?php foreach (MEDIA_CATE_NEW as $key_cate => $val_cate) {?>
    ...
    <div class="panel-heading">
        <span class="panel-title">
            ...
            <button type="button" class="btn btn-sm btn-primary"
                    onclick="location.href = '<?php echo site_url('media/add'); ?>';">
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-xs btn-labeled btn-info"
                                    onclick="location.href = '<?php echo site_url('media/edit/' . $row->id) ?>';">
...
<script type="text/javascript">
    function set_status(status) {
        ...
        if (chkId != '') {
            $('#frm #id').val(chkId);
            var action = '<?php echo site_url('media/set_status') ?>/' + status;
            // compute action here...
    ...
    function data_delete() {
        ...
        if (chkId != '') {
            if (confirm('What do you want to delete?')) {
                $('#frm #id').val(chkId);
                var action = '<?php echo site_url('media/data_delete') ?>';
                // compute action here...
                $('#frm').attr('action', action);
                $('#frm').submit();
            }
