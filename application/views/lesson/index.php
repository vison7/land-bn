    ...
    <div class="panel-heading">
        <span class="panel-title">
            ...
            <button type="button" class="btn btn-sm btn-primary"
                    onclick="location.href = '<?php echo site_url('lesson/add?course_id='. $course_id); ?>';">
        ...
        <table class="table table-bordered" style="margin-bottom:0">
            ...
            <tbody>
                ...
                foreach ($query as $row) {
                    ...
                    $thumb = '';
                    if ($row->thumb != '') {
                        $thumb = '<img src="' . base_url() . $row->thumb . '"  border="0" class="img-responsive" /><br>';
                    }
                    ?>
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-xs btn-labeled btn-info"
                                    onclick="location.href = '<?php echo site_url('lesson/edit/' . $row->id.'?course_id='. $course_id) ?>';">
...
<script type="text/javascript">
    function set_status(status) {
        ...
        if (chkId != '') {
            $('#frm #id').val(chkId);
            var action = '<?php echo site_url('lesson/set_status') ?>/' + status;
            // compute action here...
    ...
    function data_delete() {
        ...
        if (chkId != '') {
            if (confirm('What do you want to delete?')) {
                $('#frm #id').val(chkId);
                var action = '<?php echo site_url('lesson/data_delete') ?>';
                // compute action here...
                $('#frm').attr('action', action);
                $('#frm').submit();
            }
